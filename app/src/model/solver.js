window.SolverModel = {
  iterateSolver: function iterateSolver(state) {
    ...
    var g = window.GeometryModel.computeGeometry(state), f = window.ContactModel.computeContact(state);
    var wn = new Float64Array(N), un = new Float64Array(N), yn = new Float64Array(N);
    var T = Math.max(600, p.Tp + 0.002 * p.EA), Ty = Math.max(300, 0.35 * T), contactExtra = p.kc * p.b * ds, treadExtra = p.kb * p.b * ds;
    var denW = p.kr * ds + 2 * T / ds + 6 * p.EI / Math.pow(ds, 3) + 0.35 * contactExtra, denU = p.kt * ds + 2 * T / ds + 0.35 * treadExtra, denY = p.ky * ds + 2 * Ty / ds + 0.35 * treadExtra;
    var maxU = 0;
    for (var i = 0; i < N; i++) {
      ...
      var Fw = T * lapW / ds - p.EI * biW / Math.pow(ds, 3) - p.kr * w[i] * ds - p.EA * f.eps[i] * ds / Math.max(p.R0, 1e-6);
      var Fu = T * lapU / ds - p.kt * u[i] * ds - 0.12 * p.EA * f.duf[i] * ds;
      var Fy = Ty * lapY / ds - p.ky * y[i] * ds;
      if (g.pen[i] > 0) { var Fn = g.pc[i] * p.b * ds; Fw += Fn * erz; Fu += Fn * etz; var Fx = f.tx[i] * p.b * ds, Fyn = f.ty[i] * p.b * ds; Fw += Fx * erx; Fu += Fx * etx; Fy += Fyn; }
      ...
      wn[i] = clamp(w[i] + dw, -0.18 * p.R0, 0.08 * p.R0); un[i] = clamp(u[i] + du, -0.10 * p.R0, 0.10 * p.R0); yn[i] = clamp(y[i] + dy, -0.12 * p.R0, 0.12 * p.R0);
    ...
    state.solver = { iterations: 1, converged: Math.abs(err) / Math.max(p.Ntarget, 1) < 1e-3, residual: Math.abs(err) / Math.max(p.Ntarget, 1) + maxU / Math.max(0.001, p.R0) };
    return state.solver;
  }
};
